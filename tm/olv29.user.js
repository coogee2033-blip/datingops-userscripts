...
(() => {
  ...
  if (g.__olv29InitDone) {
    console.log("[OLV29] already initialized, skip duplicate load");
    return;
  }
  ...
  function updateMemoCandidateBox(memoCandidateRaw) {
    ...
    console.log("[OLV29] memo_candidate from n8n:", memoCandidateRaw, "=>", text);
    box.value = text;
  }
    ...
    const tryOne = (url) =>
      new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          ...
          timeout: 20000,
          onload: (res) => {
            console.log("[OLV29] n8n response:", url, res.status, res.responseText);
  ...
  async function sendManual() {
    ...
    try {
      ...
      console.log("[OLV29 v1.8] sending payload to n8n:", payload);
  ...
  function mountAuto() {
    ...
    (async () => {
      try {
        const payload = await buildWebhookPayload();
        console.log("[OLV29 v1.8] sending payload to n8n (auto-on-load):", payload);
        const res = await postJSONWithFallback(payload);
        ...
        console.warn("[OLV29] auto-send error:", e);
      }
    })();
  }
  ...
  (async function init() {
    ...
    if (window.__olv29Initialized) {
      console.debug("[OLV29] skip: already initialized in init()");
      return;
    }
