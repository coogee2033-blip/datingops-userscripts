/**
 * OLV29 Sync Check Script
 *
 * 「GitHub ↔ ローカル ↔ Tampermonkey」のズレをチェックするスクリプト
 *
 * 使い方:
 *   npx ts-node scripts/check-olv29-sync.ts
 *   または
 *   npm run check:olv29
 *
 * Tampermonkey との比較をする場合:
 *   1. Tampermonkey ダッシュボード → OLV29 スクリプトを開く
 *   2. エディタの内容をすべてコピー
 *   3. tmp/olv29.from-tm.js に貼り付けて保存
 *   4. このスクリプトを再実行
 */
...
// ---- 定数 ----
const LOCAL_PATH = "tm/olv29.user.js";
const REMOTE_URL =
  "https://raw.githubusercontent.com/coogee2033-blip/datingops-userscripts/main/tm/olv29.user.js";
const TM_COPY_PATH = "tmp/olv29.from-tm.js";
...
async function main(): Promise<void> {
  ...
  if (tm === null) {
    ...
    console.log("     1. TM ダッシュボード → OLV29 スクリプトを開く");
    ...
    if (localVsGithub && localVsTm && githubVsTm) {
      console.log("");
      console.log("🎉 Local / GitHub / TM (OLV29) すべて一致しています！");
    }
  }
